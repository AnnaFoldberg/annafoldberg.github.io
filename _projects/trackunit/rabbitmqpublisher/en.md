---
title: "Trackunit: RabbitMQPublisher - messaging NuGet package"
...
The **RabbitMQPublisher** in the messaging NuGet package establishes a shared, reusable, thread-safe connection to **RabbitMQ** and publishes events. `PublishAsync` ensures the connection is initialized, creates a channel, declares the exchange idempotently, and sends the event payload as JSON with persistent delivery. The class supports automatic connection recovery via `AutomaticRecoveryEnabled = true` and implements `DisposeAsync` to correctly dispose the channel and connection asynchronously.
