...
- **`analysis.started` (fanout)**
  ...
  - **Consumers:** GraphGateway (instrada verso le subscription GraphQL tramite `RabbitToSubscriptions`, queue `graph.subs.started`)

- **`analysis.completed` (fanout)**
  - **Publisher:** SvcAnalysisOrchestrator
  - **Consumers:** GraphGateway (instrada verso le subscription GraphQL tramite `RabbitToSubscriptions`, queue `graph.subs.completed`)