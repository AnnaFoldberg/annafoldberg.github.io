...
- **`analysis.started` (fanout)**
  ...
  - **Consumers:** GraphGateway (videresender til GraphQL-subscriptions via `RabbitToSubscriptions`, queue `graph.subs.started`)

- **`analysis.completed` (fanout)**
  - **Publisher:** SvcAnalysisOrchestrator
  - **Consumers:** GraphGateway (videresender til GraphQL-subscriptions via `RabbitToSubscriptions`, queue `graph.subs.completed`)