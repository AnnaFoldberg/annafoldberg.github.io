---
title: "Trackunit: RabbitMQConsumer - messaging NuGet package"
...
The **RabbitMQConsumer** in the messaging NuGet package creates a connection and channel, declares a `fanout` exchange, and binds a queue to it. `RunAsync` starts an asynchronous consume loop where incoming messages are processed via a handler function. The handler’s result determines whether the message is acked or nacked without requeue. The class supports automatic connection recovery via `AutomaticRecoveryEnabled = true` and implements `DisposeAsync` for correct asynchronous disposal of the channel and connection.
...
**RabbitMQConsumer** implements **IEventConsumer**:
